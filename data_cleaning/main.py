...
# prepare data
csv_url = 'data_cleaning/data/skodel_data_original.csv'
...
# pd.concat([df1,df2, df3],axis=1).to_excel('data_cleaning/data/class_summary_data.xlsx')

# # drop students who changed school
# student_summary_data_df = student_summary_data_df.drop(student_summary_data_df.index[10])
# student_summary_data_df = student_summary_data_df.drop(student_summary_data_df.index[10])
# student_summary_data_df.to_excel('data_cleaning/data/student_summary_data.xlsx', index=False)

# write cleaned_sentences to txt
with open("data_cleaning/data/cleaned_sentences.txt", 'w') as output:
    for row in column_9:
